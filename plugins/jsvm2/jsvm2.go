...
import (
	...
	"github.com/func-rest/space/plugins/jsvm2/internal/types/generated"
...
func Register(app core.App, config Config) error {
	...
	if p.config.HooksFilesPattern == "" {
		p.config.HooksFilesPattern = `^.*(\.pb\.js|\.cubby\.js|\.cubby\.ts|\.pb\.ts)$`
	}
...
func (p *plugin) registerHooks() error {
	...
	executors := newPool(p.config.HooksPoolSize, func(r *goja.Runtime) {
		sharedBinds(r)
	...
	for file, _ := range files {
		func() {
			...
			if len(lastResult.OutputFiles) >= 1 {

				_, err := loader.RunString(string(lastResult.OutputFiles[0].Contents))
				if err != nil {
					panic(err)
				}
			} else {
...
func (p *plugin) watchHooks() error {
	...
	dirsErr := filepath.Walk(p.config.HooksDir, func(path string, info fs.FileInfo, err error) error {
		...
		// fmt.Println("path", path, info)
		if !info.IsDir() || info.Name() == "node_modules" {
			return nil
		}
		// fmt.Println("path past", path)
		return watcher.Add(path)
	})
	if dirsErr != nil {
		fmt.Println("Dirs err", dirsErr)
		watcher.Close()
	}
